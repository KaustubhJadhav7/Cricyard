...
// Method to create the video segment
  Future<void> createNextVideoSegment(int framesNeeded) async {
    ...
    await FFmpegKit.execute(ffmpegyouTubeCommand).then((rc) async {
  ...
  final String ffmpegStreamCommand =
      '-re -i $segmentPath -f lavfi -t 1 -i anullsrc=channel_layout=stereo:sample_rate=44100 '
  ...
  try {
    ...
    final FFmpegSession session = await FFmpegKit.execute(ffmpegStreamCommand);
    ...
    if (returnCode != null && returnCode == 0) {
      ...
      print('FFmpeg command to stream to YouTube failed with rc $returnCode');
      final String? failStackTrace = await session.getFailStackTrace();
      if (failStackTrace != null) {
        print('FFmpeg failed stack trace: $failStackTrace');
      }
      _showErrorMessage(
          'FFmpeg command to stream to YouTube failed with rc $returnCode');
    }
  } catch (error) {
    print('FFmpeg execution error: $error');
    _showErrorMessage('Streaming to YouTube failed: $error');
