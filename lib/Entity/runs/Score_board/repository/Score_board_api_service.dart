...
class score_boardApiService {
  ...
  try {
    final response = await apiService.getGetApiResponse('$baseUrl/Score_board/Score_board');
...
Future<void> updateEntity(int entityId, Map<String, dynamic> entity) async {
  ...
  try {
    await apiService.getDeleteApiResponse(
        '$baseUrl/Score_board/Score_board/$entityId');
  } catch (e) {
...
Future<List<Map<String, dynamic>>> getAllTeam(int matchId) async {
  try {
    final response =
        await apiService.getGetApiResponse('$baseUrl/Match/Match/teams/$matchId');
...
Future<List<Map<String, dynamic>>> getPartnershipDetails(int matchId) async {
  try {
    final response = await apiService.getGetApiResponse(
        '$baseUrl/token/Practice/score/partnership/$matchId');
...
Future<Map<String, dynamic>> getExtrasDetails(int matchId) async {
  try {
    final response = await apiService.getGetApiResponse(
        '$baseUrl/token/Practice/score/extra/$matchId');
...
Future<void> postWideExtra(String type, int runs, int matchId, int innings,
  ...
  try {
    final url =
        '$baseUrl/runs/score/extra/$matchId/$innings/$runs/$type';
    await apiService.getPostApiResponse(url, lastRec);
  } catch (e) {
    print("Error-Updating Bowler -- $e");
  }
  ...
  try {
    final response = await apiService.getGetApiResponse('$baseUrl/team/Register_team/member/$teamId');
    if (response != null && response is List) {
...
Future<List<Map<String, dynamic>>> getScoreBoard(int matchId) async {
  try {
    final response = await apiService
        .getGetApiResponse('$baseUrl/runs/playercareer/scorecard?matchId=$matchId');
    if (response != null && response is List) {
...
Future<List<Map<String, dynamic>>> getFallOfWicket(int matchId, int inning) async {
  try {
    final response = await apiService
        .getGetApiResponse('$baseUrl/score/wicket/fall?matchId=$matchId&inning=$inning');
    if (response != null && response is List) {
      List<Map<String, dynamic>> responseData =
          response.map((item) => Map<String, dynamic>.from(item)).toList();
