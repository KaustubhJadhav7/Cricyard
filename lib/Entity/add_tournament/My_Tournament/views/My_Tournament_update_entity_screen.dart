...
class _MyTournamentUpdateEntityScreenState
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(
        ...
        centerTitle: true,
        title: AppbarTitle(text: "Update My Tournament"),
      ),
      body: SingleChildScrollView(
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Form(
            ...
            child: Column(
              children: [
                buildTextField("Description", "Enter Description",
                    widget.entity['description'], (value) {
                  widget.entity['description'] = value;
                }),
                buildTextField("Rules", "Enter Rules", widget.entity['rules'],
                    (value) {
                  widget.entity['rules'] = value;
                }),
                buildTextField("Venues", "Please Enter Venues",
                    widget.entity['venues'], (value) {
                  widget.entity['venues'] = value;
                }),
                GestureDetector(
                ...
                buildTextField("Sponsors", "Please Enter Sponsors",
                    widget.entity['sponsors'], (value) {
                  widget.entity['sponsors'] = value;
                }),
                DropdownButtonFormField<String>(
                  decoration:
                      const InputDecoration(labelText: 'Tournament Name'),
                  value: provider.selectedtournament_nameValue,
                  items: provider.tournament_nameItems.map((item) {
                    return DropdownMenuItem<String>(
                  ...
                  onChanged: (value) {
                    setState(() {
                      provider.selectedtournament_nameValue = value;
                    });
                  },
                  validator: (value) {
                    if (value == null || value.isEmpty) {
                ...
                CustomButton(
                  ...
                  onTap: () async {
                    if (_formKey.currentState!.validate()) {
                      _formKey.currentState!.save();
                      try {
                        await provider.apiService.updateEntity(
  ...
  Padding buildTextField(
      String label, String hint, String? initialValue, Function(String?) onSave) {
    return Padding(
      padding: getPadding(top: 19),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  void showErrorDialog(BuildContext context, Object error) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: const Text('Error'),
        content: Text('Failed to update My Tournament: $error'),
        actions: [
          TextButton(
            child: const Text('OK'),
